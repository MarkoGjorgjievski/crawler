...
router.addHandler(
    ...
    async ({ request, page, crawler, log }) => {
        ...
        try {
            ...
            await page.waitForSelector('ul.cat-nav');
            ...
            //     const anchors = Array.from(document.querySelectorAll('ul.cat-nav li ul.dropdown-menu li a'));

            //     return anchors.map(a => ({ name: a.innerHTML, slug: a.getAttribute('href') || '#' }))
            // });
