...
router.addHandler('EXTRACT_CATEGORIES', async ({ request, page, crawler, log }) => {
...
router.addHandler('SCRAPE_CATEGORY', async ({ request, page, crawler, log }) => {
    const { category, categorySlug, currentPage, itemsPerPage, maxPages } = request.userData;

    log.info(`Scraping category: ${category} - Page ${currentPage}/${maxPages}`);
    ...
    try {
        ...
        const products = await page.evaluate((categoryName) => {
            ...
            productContainers.forEach((container) => {
                const hoverDiv = container.querySelector('div.hover\\:cursor-pointer');
                if (!hoverDiv) return;
                ...
                const titleElement = hoverDiv.querySelector('p + h3') || hoverDiv.querySelector('h3.text-lg');
                const title = titleElement?.textContent?.trim() || '';

                // Club price
                const clubPriceElement = container.querySelector('p.text-orange.text-sm');
                let clubPrice = '';
                ...
                // Regular price
                const regularPriceElement = container.querySelector('p.text-blackSecondary.text-xs');
                let regularPrice = '';
                ...
                const img = container.querySelector('img.object-cover') || container.querySelector('img');
        ...
        log.info(`Processing ${productsToProcess.length} products (itemsPerPage: ${itemsPerPage})`);
        ...
        if (currentPage >= maxPages) {
            log.info(`Reached max pages limit (${maxPages}) for category: ${category}`);
            return;
        }

        // Check for next page button
        const hasNextPage = await page.evaluate(() => {
            const nextButton = document.querySelector('button:not([disabled]) > img[alt="Arrow right"]');
        ...
        error && log.error(`Error scraping category ${category} page ${currentPage}:`, error);
        throw error;
    }
});
...
router.addHandler('GET_PRODUCT_URL', async ({ request, page, log }) => {
    ...
    try {
        ...
        const productUrl = await page.evaluate(() => {
            const resultsContainer = document.querySelector('div.absolute.w-full.max-w-md');
        ...
        const closeButton = await page.$('button[aria-label="Close search results"]');
        if (closeButton) {
